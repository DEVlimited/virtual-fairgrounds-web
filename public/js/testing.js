...
// This replaces all three of your existing systems with one coordinated approach
...
class UnifiedPerformanceManager {
    constructor(camera, renderer) {
        ...
        // Master registry of all managed objects
        this.managedObjects = new Map();
        
        // Texture cache - stores all texture variants
        ...
        // Quality thresholds - these determine when to switch quality levels
        this.qualityThresholds = {
            high: 40,      // Full quality within 40 units
            medium: 100,   // Medium quality from 40-100 units  
            low: 200,      // Low quality from 100-200 units
            cull: 300      // Hide beyond 300 units
        };
        
        console.log('Unified Performance Manager initialized');
    }

    // Register an object for comprehensive management
    // This is your single entry point for all performance optimization
    registerObject(mesh, options = {}) {
        const id = mesh.uuid;
        
        // Set up object-specific configuration
        const config = {
            mesh: mesh,
            type: options.type || 'generic', // 'road', 'building', 'detail', 'terrain'
            
            // Custom thresholds for different object types
            thresholds: this.getThresholdsForType(options.type),
            ...
            useQualityScaling: options.useQualityScaling !== false
        };
        
        // Initialize all the variants we'll need
        this.initializeObjectVariants(config);
        
        // Store in our registry
        this.managedObjects.set(id, config);
        
        console.log(`Registered ${config.type} object: ${mesh.name}`);
        return id;
    }

    // Get appropriate quality thresholds based on object type
    // Different types of objects need different treatment
    getThresholdsForType(type) {
        switch (type) {
            case 'road':
            case 'terrain':
                // Roads and terrain should be visible from far away
                return {
                    high: 100,
                    medium: 300,
                    low: 600,
            ...
            case 'building':
                // Buildings are important mid-range objects
                return {
                    high: 50,
                    medium: 150,
                    low: 300,
            ...
            case 'detail':
                // Small details can be culled aggressively
                return {
                    high: 25,
                    medium: 60,
                    low: 120,
            ...
            default:
                // Generic objects use standard thresholds
                return { ...this.qualityThresholds };
        }
    }

    // Initialize all quality variants for an object
    // This is where we prepare all the different versions we might need
    initializeObjectVariants(config) {
        const { mesh, originalMaterial } = config;
        
        // Create material variants that preserve lighting behavior
        // Instead of switching material types, we modify properties
        config.materialVariants = {
            high: originalMaterial,
            medium: this.createMediumQualityMaterial(originalMaterial),
            low: this.createLowQualityMaterial(originalMaterial)
        };
        
        // Process textures for this object
        this.initializeTextureVariants(config);
    }

    // Create medium quality material variant
    // Key principle: Keep the same material type but reduce complexity
    createMediumQualityMaterial(originalMaterial) {
        if (!originalMaterial || !this.isMaterialOptimizable(originalMaterial)) {
            return originalMaterial;
        }
        
        // Clone the original material to preserve its type and core properties
        const mediumMaterial = originalMaterial.clone();
        
        // Reduce quality settings while keeping the same material type
        if (mediumMaterial.roughness !== undefined) {
            mediumMaterial.roughness = Math.min(mediumMaterial.roughness + 0.2, 1.0);
        }
        
        if (mediumMaterial.metalness !== undefined) {
            mediumMaterial.metalness = Math.max(mediumMaterial.metalness - 0.1, 0.0);
        }
        
        // Reduce normal map influence but don't remove it entirely
        if (mediumMaterial.normalScale) {
            mediumMaterial.normalScale = mediumMaterial.normalScale.clone().multiplyScalar(0.7);
        }
        
        return mediumMaterial;
    }

    // Create low quality material variant
    // Still preserve material type but heavily simplify
    createLowQualityMaterial(originalMaterial) {
        if (!originalMaterial || !this.isMaterialOptimizable(originalMaterial)) {
            return originalMaterial;
        }
        
        const lowMaterial = originalMaterial.clone();
        
        // Maximize roughness to eliminate reflections
        if (lowMaterial.roughness !== undefined) {
            lowMaterial.roughness = 1.0;
        }
        
        // Remove metallic properties
        if (lowMaterial.metalness !== undefined) {
            lowMaterial.metalness = 0.0;
        }
        
        // Disable expensive maps for distant objects
        // But keep the base color/diffuse map
        if (lowMaterial.normalMap) {
            lowMaterial.normalMap = null;
        }
        
        if (lowMaterial.roughnessMap) {
            lowMaterial.roughnessMap = null;
        }
        
        if (lowMaterial.metalnessMap) {
            lowMaterial.metalnessMap = null;
        }
        
        return lowMaterial;
    }

    // Check if a material can be optimized
    isMaterialOptimizable(material) {
        return (
    ...
    // Initialize texture variants for an object
    // This creates different resolution versions of each texture
    initializeTextureVariants(config) {
        const materials = Array.isArray(config.originalMaterial) ? 
            config.originalMaterial : [config.originalMaterial];
        
        materials.forEach(material => {
            if (!material || !this.isMaterialOptimizable(material)) return;
            ...
            textureProperties.forEach(prop => {
                if (material[prop]) {
                    this.processTextureForQuality(material[prop], config, prop);
                }
            });
        });
    }

    // Process a single texture to create quality variants
    processTextureForQuality(texture, config, propertyName) {
        if (!texture.image || this.textureCache.has(texture.uuid)) {
            return; // Already processed or invalid
        }
        
        const variants = {
            high: texture, // Original texture
            medium: this.createMediumQualityTexture(texture),
            low: this.createLowQualityTexture(texture)
        };
        
        // Cache the variants
        this.textureCache.set(texture.uuid, variants);
        
        // Store reference in object config
        if (!config.textureVariants[propertyName]) {
            config.textureVariants[propertyName] = variants;
        }
    }

    // Create medium quality texture (half resolution)
    createMediumQualityTexture(originalTexture) {
        return this.resizeTexture(originalTexture, 0.5);
    }

    // Create low quality texture (quarter resolution)
    createLowQualityTexture(originalTexture) {
        return this.resizeTexture(originalTexture, 0.25);
    }

    // Utility function to resize textures
    resizeTexture(texture, scale) {
        ...
        try {
            ...
            const newWidth = Math.max(1, Math.floor(texture.image.width * scale));
            const newHeight = Math.max(1, Math.floor(texture.image.height * scale));
            ...
            this.copyTextureProperties(texture, newTexture);
            
            return newTexture;
        } catch (error) {
            console.warn('Failed to resize texture, using original:', error);
            return texture;
        }
    }

    // Copy texture properties to maintain consistency
    copyTextureProperties(source, target) {
        const properties = [
            'wrapS', 'wrapT', 'minFilter', 'magFilter', 'colorSpace',
            'flipY', 'premultiplyAlpha', 'generateMipmaps', 'anisotropy'
        ];
    ...
    // This is where all the coordination happens
    update() {
        this.stats.frameCount++;
        
        // Reset frame stats
        this.stats.totalObjects = this.managedObjects.size;
        this.stats.culledObjects = 0;
        this.stats.highQuality = 0;
        this.stats.mediumQuality = 0;
        this.stats.lowQuality = 0;
        
        // Update camera frustum for culling
        this.updateCameraFrustum();
        
        // Process each managed object
        for (const [id, config] of this.managedObjects) {
            this.updateObject(config);
        }
        
        // Log stats occasionally
        if (this.stats.frameCount % 120 === 0) {
    ...
    // Update a single object's quality and visibility
    // This is where the magic happens - coordinated quality management
    updateObject(config) {
        const { mesh, thresholds, allowCulling, useQualityScaling } = config;
        ...
        // Step 1: Frustum culling check
        if (allowCulling && !this.isObjectInFrustum(mesh)) {
            mesh.visible = false;
        ...
        // Object is visible, ensure it's enabled
        mesh.visible = true;
        config.isVisible = true;
        
        // Step 2: Determine quality level based on distance
        if (useQualityScaling) {
            const distance = this.camera.position.distanceTo(mesh.position);
            const newQuality = this.determineQualityLevel(distance, thresholds);
            
            // Step 3: Apply quality changes if needed
            if (newQuality !== config.currentQuality) {
                this.applyQualityLevel(config, newQuality);
                config.currentQuality = newQuality;
            }
        }
        
        // Update stats
        this.updateStatsForObject(config);
    }

    // Determine what quality level an object should use
    determineQualityLevel(distance, thresholds) {
    ...
    // Apply a quality level to an object
    // This coordinates both material and texture changes
    applyQualityLevel(config, qualityLevel) {
        const { mesh, materialVariants, textureVariants } = config;
        
        if (qualityLevel === 'cull') {
            mesh.visible = false;
            config.isVisible = false;
            return;
        }
        ...
        if (materialVariants[qualityLevel]) {
            mesh.material = materialVariants[qualityLevel];
        }
        
        // Apply texture variants to the current material
        this.applyTextureVariants(mesh.material, textureVariants, qualityLevel);
    ...
    // Apply texture variants to a material
    applyTextureVariants(material, textureVariants, qualityLevel) {
        if (!material || !this.isMaterialOptimizable(material)) return;
        
        // Apply each texture variant
        Object.keys(textureVariants).forEach(propertyName => {
            const variants = textureVariants[propertyName];
            if (variants && variants[qualityLevel] && material[propertyName]) {
                material[propertyName] = variants[qualityLevel];
            }
        });
    }
    ...
    isObjectInFrustum(mesh) {
        // Use bounding box test for frustum culling
        return this.frustum.intersectsObject(mesh);
    }

    // Update performance statistics
    updateStatsForObject(config) {
    ...
    logPerformanceStats() {
        ...
        console.log('Performance Manager Stats:', {
            'Total Objects': this.stats.totalObjects,
            'Culled': `${this.stats.culledObjects} (${cullingEfficiency}%)`,
            'High Quality': this.stats.highQuality,
            'Medium Quality': this.stats.mediumQuality,
            'Low Quality': this.stats.lowQuality,
    ...
    dispose() {
        ...
        for (const [id, config] of this.managedObjects) {
            Object.values(config.materialVariants).forEach(material => {
                if (material && material !== config.originalMaterial) {
                    material.dispose();
                }
            });
        }
        ...
        for (const [id, variants] of this.textureCache) {
            Object.values(variants).forEach(texture => {
...
function main() {
    ...
    function debugMaterialChanges(camera, scene) {
        const debugInfo = {
            materials: new Map(),
            previousMaterials: new Map(),
            frameCount: 0
        };
        
        return function updateDebugInfo() {
            debugInfo.frameCount++;
            
            // Only check every 30 frames to avoid spam
            if (debugInfo.frameCount % 30 !== 0) return;
            ...
            scene.traverse((child) => {
                if (child.isMesh && child.material && child.name.toLowerCase().includes('tree')) {
                    const meshId = child.uuid;
                    const currentMaterial = child.material;
                    const distance = camera.position.distanceTo(child.position);
                    
                    // Check if material changed
                    const previousMaterial = debugInfo.previousMaterials.get(meshId);
                    if (previousMaterial && previousMaterial !== currentMaterial) {
                        console.log(`MATERIAL CHANGE DETECTED!`);
                        console.log(`Tree: ${child.name}`);
                        console.log(`Distance: ${distance.toFixed(2)}`);
                        console.log(`Previous Material Type: ${previousMaterial.type}`);
                        console.log(`New Material Type: ${currentMaterial.type}`);
                        console.log(`---`);
                    }
                    
                    debugInfo.previousMaterials.set(meshId, currentMaterial);
                }
            });
        };
    }
    ...
    const performanceManager = new UnifiedPerformanceManager(camera, renderer);
    ...
    // GLTF Model loading with improved error handling
    {
        ...
        gltfLoader.load(baseURL + 'fairgrounds.glb',(glb) => {
            try {
                ...
                // Register objects with the unified system
                let meshCount = 0;
                const registeredObjects = [];

                console.log('=== REGISTERING OBJECTS WITH UNIFIED SYSTEM ===');
                ...
                root.traverse((child) => {
                    if (child.isMesh && child.material) {
                        ...
                        // Determine object type and configuration
                        let objectType = 'generic';
                        let objectConfig = {};
                        
                        // Classify objects for appropriate treatment
                        if (name.includes('roads') || name.includes('Plane') || 
                            (child.material && child.material.name && 
                            child.material.name.toLowerCase().includes('road'))) {
                            objectType = 'road';
                            objectConfig = {
                                type: 'road',
                                allowCulling: false, // Roads should always be visible
                                useQualityScaling: true
                            };
                        }
                        else if (name.includes('building') || name.includes('structure') ||
                                name.includes('house') || name.includes('tower')) {
                            objectType = 'building';
                            objectConfig = {
                                type: 'building',
                                allowCulling: true,
                        ...
                        else if (name.includes('detail') || name.includes('decoration') ||
                                name.includes('prop') || name.includes('ornament')) {
                            objectType = 'detail';
                            objectConfig = {
                                type: 'detail',
                                allowCulling: true,
                        ...
                        else if (name.includes('ground') || name.includes('terrain') ||
                                name.includes('landscape')) {
                            objectType = 'terrain';
                            objectConfig = {
                                type: 'terrain',
                                allowCulling: false, // Terrain should stay visible
                                useQualityScaling: true
                            };
                        }
                        ...
                        try {
                            const objectId = window.performanceManager.registerObject(child, objectConfig);
                        ...
                        // Apply fog shaders (keep this part as is)
                        try {
                ...
                console.log(`Registered ${registeredObjects.length} objects with unified performance manager`);
    ...
    function render(time) {
        try {
            ...
            if (window.performanceManager) {
                try {
                    window.performanceManager.update();
                } catch (error) {
                    console.warn('Performance manager update error:', error);
                }
            }

            const debugUpdate = debugMaterialChanges(camera, scene);
