...
//I found this amazing fog setup from this youtube video (https://www.youtube.com/watch?v=k1zGz55EqfU),
//I watched it and was in awe because the fog looks A M A Z I N G 
//The basics of what he did was he took the main library of the fog from THREE and edited a bit so that it adds
//cabability to control the height of the fog. With that you are still able to see the sky box
//But it makes the fog seem realistic. The noise thing idk if it fully works, its hard to tell.
//But the noise is mostly like a fog animation kind of thing, its pretty cool if it works....
class FogDemo {
  constructor() {
    this.Initialize_();
  }
  ...
  Initialize_() {
	...
	this.baseURL = 'https://storage.googleapis.com/fairgrounds-model/';
	...
	loader.load( imagePath, (panorameTexture) => {
			...
			skySphereMesh.material.onBeforeCompile = ModifyShader_;
			this.scene_.add(skySphereMesh);
		}
	);

	const planeSize = 40;

	const texture = loader.load( this.baseURL + 'Image.png' );
	...
	mesh.material.onBeforeCompile = ModifyShader_;
	this.scene_.add( mesh );

	this.gltfLoader = new GLTFLoader();
	this.gltfLoader.load( this.baseURL + 'fairgrounds.gltf', ( gltf ) => {
    ...
    this.scene_.fog = new THREE.FogExp2(0xbe9fd4, 0.005);
